...
const downloadLastYearHistoryCache = new Map();

export type LastYearRangeResponse = {
...
export const getLastYearDownloads = async (
  supabase: SupabaseClient,
  name: NpmPackageName,
  signal?: AbortSignal
): Promise<LastYearRangeResponse> => {
  const key = `downloads:last-year:${name}`;

  return cachedAsync(
    downloadLastYearHistoryCache,
    key,
    { ttlMs: DAY_MS },
    async () => {
      try {
        ...
        if (error) {
          // invoke-level error (network, gateway, etc.)
          throw new HttpError(0, 'npm-downloads-last-year', error.message);
        }

        if (!data?.ok) {
          const status = Number(data?.status ?? 0);
          const url = String(data?.url ?? 'npm-downloads-last-year');
          const rawBody =
        ...
        return data.data as LastYearRangeResponse;
      } catch (e) {
        if (e instanceof DOMException && e.name === 'AbortError') throw e;

        if (e instanceof HttpError) throw e;

        const url = `npm-downloads-last-year:${name}`;
        const message =
          e instanceof Error
            ? e.message
            : typeof e === 'string'
